...
import { CityHomeToggle } from "@/components/admin/places/city-home-toggle";
...
export default async function AdminCitiesPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <AdminPageHeader
        title="Cities"
        description="Each of these has a page of its own and a homes-for-sale page. The ones marked as search cities appear in the search filter; the switch on each card decides whether it appears on the home page."
      />
            ...
            <li key={city.id}>
              <div className="flex h-full flex-col gap-3 admin-card p-5 shadow-xs">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {city.inSearch ? (
                    <Badge tone="outline">In search</Badge>
                  ) : null}
                </div>
                ...
                <dl className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-foreground-muted">
                  <div className="flex gap-1.5">
                    ...
                    <dd
                      className={cn(
                        "font-medium",
                        thin ? "text-warning" : "text-foreground",
                      )}
                    >
                ...
                {/*
                  The home-page switch sits ON THE CARD, not inside the editor:
                  choosing which cities lead the front page is a decision made
                  while looking at all of them together.
                */}
                <div className="mt-auto border-t border-border pt-3">
                  <CityHomeToggle
                    id={city.id}
                    name={city.name}
                    showOnHome={city.showOnHome}
                  />
                </div>
